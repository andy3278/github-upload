...
def answer_five():
    import numpy as np
    Top15 = answer_one()
    answer = Top15['Energy Supply per Capita'].mean(axis=0)
    return answer
answer_five()
...
def answer_six():
    import numpy as np 
    Top15 = answer_one()
    Renewable = Top15['% Renewable'].argmax()
    return (Renewable, Top15.loc[Renewable, '% Renewable'])
answer_six()
...
def answer_seven():
    ...
    ratio = (Top15['Self-citations']/Top15['Citations']).argmax()
    return (ratio, Top15.loc[ratio, 'CitationRatio'])
answer_seven()
